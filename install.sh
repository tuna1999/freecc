...
# Free CC installer
# Usage: curl -fsSL https://raw.githubusercontent.com/chat812/freecc/main/install.sh | bash
...
REPO="https://github.com/chat812/freecc.git"
INSTALL_DIR="$HOME/freecc"
...
build_binary() {
  info "Building freecc (all experimental features enabled)..."
  cd "$INSTALL_DIR"
  bun run build:dev:full
  # build:dev:full emits the dev binary at ./freecc-dev per CLAUDE.md.
  ok "Binary built: $INSTALL_DIR/freecc-dev"
}
...
link_binary() {
  ...
  ln -sf "$INSTALL_DIR/freecc-dev" "$link_dir/freecc"
  ok "Symlinked: $link_dir/freecc"
...
printf "    ${CYAN}freecc${RESET}                          # interactive REPL\n"
printf "    ${CYAN}freecc -p \"your prompt\"${RESET}          # one-shot mode\n"
echo ""
printf "  ${BOLD}Set your API key:${RESET}\n"
printf "    ${CYAN}export ANTHROPIC_API_KEY=\"sk-ant-...\"${RESET}\n"
echo ""
printf "  ${BOLD}Or log in with Claude.ai:${RESET}\n"
printf "    ${CYAN}freecc /login${RESET}\n"
echo ""
printf "  ${DIM}Source: $INSTALL_DIR${RESET}\n"
printf "  ${DIM}Binary: $INSTALL_DIR/freecc-dev${RESET}\n"
printf "  ${DIM}Link:   ~/.local/bin/freecc${RESET}\n"
echo ""
