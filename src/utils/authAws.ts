/**
 * AWS-specific authentication helpers.
 *
 * Extracted from `auth.ts` to reduce file size. Public API is re-exported
 * from `auth.ts` so callers don't need to update imports.
 *
 * Owns:
 * - awsAuthRefresh / awsCredentialExport settings access
 * - AWS STS caller identity checks
 * - AWS credential refresh + cache management
 */
