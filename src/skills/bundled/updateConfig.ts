...
|------|-------|-----|---------|
| \`~/.freecc/settings.json\` | Global | N/A | Personal preferences for all projects |
...
\`\`\`json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "jq -r '.tool_input.command' >> ~/.freecc/bash-log.txt"
      }]
    }]
  }
}
...
1. **Check the settings file** - Read ~/.freecc/settings.json or .claude/settings.json
